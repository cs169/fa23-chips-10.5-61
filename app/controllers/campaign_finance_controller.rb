...
class CampaignFinanceController < ApplicationController
  ...
  def search
    ...
    if @available_categories.include?(selected_category) && @available_cycles.include?(selected_cycle)
  ...
  def set_selected_category_and_api(selected_cycle, selected_category)
    ctg_mapping = {
      'Candidate Loan' => 'candidate_loans', 'Contribution Total' => 'total_contributions',
      'Debts Owed' => 'debts_owed', 'Disbursements Total'=> 'total_disbursements',
      'End Cash'=> 'end_cash','Individual Total'=> 'total_from_individuals',
      'PAC Total'=> 'total_from_pacs', 'Receipts Total'=> 'total_receipts', 'Refund Total'=> 'total_refunds'
    }
    @selected_category_api = ctg_mapping[selected_category]
